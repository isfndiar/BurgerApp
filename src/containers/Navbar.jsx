...
function Navbar() {
  ...
  const classNav = 'fixed top-0 left-0 right-0 flex justify-around text-black text-[1.2rem] py-[1.5rem] px-[5%] z-50 transition-all ease-in-out  duration-500 backdrop-blur-xl';
...
const ListMenu = ({isActive}) => {
  ...
  return (
    <div className={`ms-[20px] flex gap-[50px] ${isActive ? "active" : ""} font-light`}>
    <a href={"#menu"} className={`hover:text-blue-500`}>
      Menu
    </a>
    <a href={`#food`} className={`hover:text-blue-500`} >Food</a>
